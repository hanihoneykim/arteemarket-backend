...
def kakao_get_access_token(data):
    ...
    error_description = kakao_token_response.get("error_description", None)
    error_code = kakao_token_response.get("error_code", None)
    if error:
        print("error", error)
...
def naver_get_user(access_token=None):
    ...
    user, _ = USER_MODEL.objects.get_or_create(
        social_provider="naver", social_uid=naver_response["id"]
    )
